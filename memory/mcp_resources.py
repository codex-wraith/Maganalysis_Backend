...
def register_memory_resources(mcp_instance, agent_instance):
    """Register memory resources with the MCP server"""
    global agent
    agent = agent_instance
    
    # Register resources with MCP
    mcp_instance.resource(f"{CUSTOM_SCHEME}:///memory/stats")(get_memory_stats)
    mcp_instance.resource(f"{CUSTOM_SCHEME}:///memory/user/{{user_id}}")(get_user_memory)