...
# Import modules with aliases for function calls
from market import mcp_tools as market_mcp_tools
from market import mcp_price_levels as market_mcp_price_levels
from market import mcp_multi_timeframe as market_mcp_multi_timeframe
...
class CipherAgent:
    ...
    async def analyze_asset(self, symbol, asset_type="stock", market="USD", interval="60min", for_user_display=False, platform="web"):
        ...
        try:
            ...
            try:
                ...
                primary_tech_data = await market_mcp_tools.get_technical_indicators(symbol, asset_type, interval)
                ...
                if current_price:
                    ...
                    mtf_analysis = await market_mcp_multi_timeframe.analyze_multi_timeframe(symbol, asset_type, interval)
                    price_levels = await market_mcp_price_levels.get_price_levels(symbol, asset_type, interval)
            ...
            tech_data = await market_mcp_tools.get_technical_indicators(symbol, asset_type, interval)
            price_levels = await market_mcp_price_levels.get_price_levels(symbol, asset_type, interval)
            sentiment = await market_mcp_tools.get_news_sentiment(symbol)
    ...
    async def _fetch_real_time_price(self, symbol: str, asset_type: str, fallback_price=None):
        ...
        try:
            ...
            tech_data = await market_mcp_tools.get_technical_indicators(symbol, asset_type, "60min")
    ...
    async def _fetch_market_data(self, symbol, asset_type, interval):
        ...
        try:
            # Get raw market data
            market_data = await market_mcp_tools.get_raw_market_data(symbol, asset_type, interval)
    ...
    async def _analyze_multi_timeframe_levels(self, symbol, asset_type, current_price, main_interval, latest_atr=None):
        ...
        try:
            ...
            # Get comprehensive price level data across timeframes
            mtf_analysis = await market_mcp_multi_timeframe.analyze_multi_timeframe(symbol, asset_type, main_interval)
            price_levels = await market_mcp_price_levels.get_price_levels(symbol, asset_type, main_interval)
    ...
    async def search_for_asset(self, query: str, search_type: str = "web", max_results: int = 5):
        ...
        try:
            ...
            if potential_symbol:
                # Try to get price data to validate the symbol
                try:
                    # Check if it's a stock
                    stock_data = await mcp.tools.get_technical_indicators(potential_symbol, "stock", "daily")
                ...
                try:
                    crypto_data = await mcp.tools.get_technical_indicators(potential_symbol, "crypto", "daily")
            ...
            search_results = await mcp.tools.search_market_information(query, search_type)
    ...
    async def process_trend_following_strategy(self, symbol: str, asset_type: str = "stock", timeframe: str = "daily"):
        ...
        try:
            ...
            # Get multi-timeframe analysis for broader context
            mtf_analysis = await mcp.tools.analyze_multi_timeframe(symbol, asset_type, timeframe)
    ...
    async def generate_market_strategy(self, symbol: str, asset_type: str = "stock", timeframe: str = "daily"):
        ...
        try:
            ...
            # Get news sentiment
            sentiment_data = await mcp.tools.get_news_sentiment(symbol)
